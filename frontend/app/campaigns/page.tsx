    ...
    <div>
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Campaign ROI: predicted revenue vs ad spend
        </p>
      </div>

      {campaigns.length === 0 ? (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-8 text-gray-500 text-sm">
          No campaign data. Run the campaign model first.
        </div>
      ) : (
        <div className="space-y-6">
          ...
          {/* Detail table — scrollable on small screens */}
          <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[700px]">
                <thead className="border-b border-gray-800">
                  <tr className="text-gray-500">
                    <th className="text-left px-4 py-3 font-medium whitespace-nowrap">Campaign</th>
                    <th className="text-right px-4 py-3 font-medium whitespace-nowrap">Spend</th>
                    <th className="text-right px-4 py-3 font-medium whitespace-nowrap">Players</th>
                    <th className="text-right px-4 py-3 font-medium whitespace-nowrap">Avg LTV</th>
                    <th className="text-right px-4 py-3 font-medium whitespace-nowrap">Revenue (90d)</th>
                    <th className="text-right px-4 py-3 font-medium whitespace-nowrap">ROAS</th>
                    <th className="text-right px-4 py-3 font-medium whitespace-nowrap">Payback</th>
                    <th className="text-right px-4 py-3 font-medium whitespace-nowrap">Status</th>
                  </tr>
                </thead>
                <tbody>
                  {campaigns.map((c) => (
                    <tr key={c.campaign_id} className="border-b border-gray-800/50 hover:bg-gray-800/40 transition-colors">
                      <td className="px-4 py-3 text-gray-200 font-medium capitalize whitespace-nowrap">
